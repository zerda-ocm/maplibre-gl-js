    ...
    for (const originalSection of formattedText.sections) {
        ...
        while ((match = markerRegex.exec(sectionText)) !== null) {
            ...
            if (lastIndex < matchIndex) {
                updatedSections.push({ ...originalSection, text: sectionText.substring(lastIndex, matchIndex), textColor: currentColor });
            }
            const named = match[1];
            ...
            if (named) {
                updatedSections.push({ ...originalSection, text: named, textColor: currentColor });
        ...
        if (lastIndex < sectionText.length) {
            updatedSections.push({ ...originalSection, text: sectionText.substring(lastIndex), textColor: currentColor });
        }
    }
    ...
    for (const originalSection of formattedText.sections) {
        ...
        while (i < sectionText.length) {
            ...
            if (splitChars.has(ch)) {
                const end = i + 1;
                if (lastIndex < end) {
                    updatedSections.push({ ...originalSection, text: sectionText.substring(lastIndex, end), textColor: currentColor });
                }
            ...
            if (ch === RGB || ch === RGBA) {
                ...
                if (match) {
                    if (lastIndex < i) {
                        updatedSections.push({ ...originalSection, text: sectionText.substring(lastIndex, i), textColor: currentColor });
                    }
                    const hex = match[0];
                    const parsed = parseHexColor(hex);
                    if (parsed) currentColor = parsed;
